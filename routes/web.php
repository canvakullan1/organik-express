...
use App\Http\Controllers\Admin\ImageFieldController;
use App\Http\Controllers\Admin\ProductImageController;
use Illuminate\Support\Facades\Route;

Route::get('/', HomeController::class)->name('home');

// Görsel yönetimi — BASİT klasik yükleme/silme (Filament FilePond yerine, güvenilir).
// Sadece panele erişebilen kullanıcılar; standart multipart form POST. (web grubu otomatik.)
Route::middleware('auth')->group(function () {
    // Ürün görselleri (çoklu, relation)
    Route::prefix('gorsel-yonet')->name('admin.product-images.')->group(function () {
    ...
    // Tekli görsel alanları (kategori, banner, üretici, marka, kutu, blog kapağı)
    Route::prefix('gorsel-tek')->name('admin.image-field.')->group(function () {
        Route::get('/{key}/{id}', [ImageFieldController::class, 'show'])->name('show');
        Route::post('/{key}/{id}', [ImageFieldController::class, 'store'])->name('store');
        Route::delete('/{key}/{id}', [ImageFieldController::class, 'destroy'])->name('destroy');
    });
});
