...
/**
 * ekotime.com.tr (ekmek/erişte/simit) + tazedirekt/migros (yumurta) kataloğunu
 * elle yazılmış SEO içerikle içe aktarır.
 *
 * Veri kaynakları (her ikisi de repoda):
 *  - database/data/ekotime-raw.json  → kaynaktan çekilen fiyat/görsel/SKU
 *  - database/data/ekotime/          → elle yazılan kategori + SEO metin parçaları
 *
 * Kullanım:
 *   php artisan import:ekotime                 # hepsini içe aktar
 *   php artisan import:ekotime --skip-images   # görselleri indirme
 *   php artisan import:ekotime --only=slug-a,slug-b
 */
class ImportEkotime extends Command
{
    protected $signature = 'import:ekotime {--skip-images} {--only=} {--status=active} {--limit=0} {--reimages}';

    protected $description = 'ekotime + yumurta kataloğunu SEO içerikli olarak içe aktarır';

    public function handle(): int
    {
        $rawPath = database_path('data/ekotime-raw.json');
        $partsDir = database_path('data/ekotime');

        if (! is_file($rawPath) || ! is_dir($partsDir)) {
            $this->error('Veri bulunamadı (ekotime-raw.json / ekotime/ parça klasörü).');
        ...
        // 1) Kategoriler — mevcutlara DOKUNMA (organikgiller tanımları ezilmesin), yoksa oluştur
        $catIdBySlug = [];
