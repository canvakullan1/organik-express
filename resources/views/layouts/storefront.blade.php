...
<body class="min-h-screen flex flex-col antialiased">
        ...
        <div class="border-b border-leaf-800">
            <div class="mx-auto max-w-7xl px-4 py-8 grid grid-cols-2 md:grid-cols-4 gap-6">
                @foreach([
                    ...
                    ['M3 13.125C3 12.504 3.504 12 4.125 12h2.25c.621 0 1.125.504 1.125 1.125v6.75C7.5 20.496 6.996 21 6.375 21h-2.25A1.125 1.125 0 0 1 3 19.875v-6.75ZM9.75 8.625c0-.621.504-1.125 1.125-1.125h2.25c.621 0 1.125.504 1.125 1.125v11.25c0 .621-.504 1.125-1.125 1.125h-2.25a1.125 1.125 0 0 1-1.125-1.125V8.625ZM16.5 4.125c0-.621.504-1.125 1.125-1.125h2.25C20.496 3 21 3.504 21 4.125v15.75c0 .621-.504 1.125-1.125 1.125h-2.25a1.125 1.125 0 0 1-1.125-1.125V4.125Z', 'Özenli Paketleme', 'Ürüne uygun güvenli paket'],
                ] as [$icon, $title, $sub])
    ...
    {{-- Mobil alt aksiyon çubuğu için boşluk: içerik sabit çubuğun altında kalmasın.
         Footer'la aynı renk (koyu yeşil) → çubuğun üstünde beyaz şerit oluşmaz. --}}
    <div class="lg:hidden bg-leaf-900" style="height:calc(3.5rem + env(safe-area-inset-bottom));" aria-hidden="true"></div>
        ...
        <div class="lg:hidden fixed inset-x-0 bottom-0 z-40 flex items-stretch border-t border-black/10 shadow-[0_-6px_20px_-8px_rgba(0,0,0,0.35)]">
            @if($waBar)
                <a href="https://wa.me/{{ $waBar }}?text={{ rawurlencode('Merhaba, Organik Express hakkında bilgi almak istiyorum.') }}"
                   ...
                   class="flex flex-1 items-center justify-center gap-2 pt-3.5 text-white font-700 tracking-wide active:brightness-95 transition"
                   style="background:linear-gradient(135deg,#25D366 0%,#1ebe5d 60%,#12a350 100%);padding-bottom:calc(0.875rem + env(safe-area-inset-bottom));"
            ...
            @if($telBar)
                <a href="tel:{{ $telBar }}"
                   class="flex flex-1 items-center justify-center gap-2 pt-3.5 bg-leaf-700 text-white font-700 tracking-wide active:brightness-95 transition"
